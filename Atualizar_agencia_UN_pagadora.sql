...
--Alteardo no dia 19/07/07 - inclusão da cláusula "and banco=1" no select da agência
 CURSOR C_CCUnPagador IS
      select sb.handle handle_benef,
             scf.handle handle_contafin,
             1 banco,
             case fl.handle --antes do hífen - código da agência c/ 4 dígitos mais o DV - após o hífen -conta-corrente mais o DV
...
begin
   ...
   FOR CAUX IN C_CCUnPagador loop
       --armazena o handle da agência da unidade pagadora
       select handle into nu_agencia_pagadora
         from sfn_agencia
        where agencia=substr(caux.ag_e_dv_contacorrente_E_dv,1,4)
          and banco=1
          and situacao = 'A';
       --verifica se o handle da agência da unidade pagadora é diferente da agência do benef
